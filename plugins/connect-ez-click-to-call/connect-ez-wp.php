...
function connect_ez_register_settings()
{
    ...
    add_settings_section('caller_callee_settings', 'Caller/Callee Settings', 'connect_ez_plugin_section_caller_callee', 'connect_ez_plugin');
...
{
    $newInput = $input;
    $newInput['realm'] = trim($input['realm']);
    return $newInput;
}
...
function connect_ez_plugin_setting_form_field($args)
{
    ...
    // if (isset($args['help'])) {
        ...
        echo '<div>Need help? Please click <a href="' . esc_url($args['help']) . '" target="_blank">here</a> for more information.</div>';
    }
